...
The <<sql-search-api,SQL search API>> accepts SQL queries in a JSON document, runs
the query, and returns results.
...
To clear the state earlier, you can use the <<sql-clear-cursor-api,SQL clear
cursor API>>:
...
returning a response. However, complete results can take longer for searches
across large data sets or <<data-tiers,frozen>> data.
...
If the request doesn't finish within the timeout period, the search becomes
async. In the CSV, TSV, or TXT format, the request returns the following HTTP
headers:

* An `Async-ID` for the search
* An `Async-partial` value of `true`, indicating the search results are
  incomplete
* An `Async-running` value of `true`, indicating the search is ongoing

// TODO: Add response

In the JSON or YAML response format, async searches return these values in the
respective `id`, `is_partial`, and `is_running` response body properties.
