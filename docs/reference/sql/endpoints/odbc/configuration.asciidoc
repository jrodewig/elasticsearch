...
page. This corresponds to {es-sql}'s request parameter
<<sql-fetch-size,`fetch_size`>>. The value 0 means server default.
+
* Max page length (MB)
+
...
This corresponds to {es-sql}'s
<<sql-field-multi-value-leniency,`field_multi_value_leniency`>> parameter.
+
